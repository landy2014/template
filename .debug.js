/*! <DEBUG:undefined> */
function anonymous($data,$filename) {'use strict';var $utils=this,$helpers=$utils.$helpers,$escape=$utils.$escape,doctype=$data.doctype,html=$data.html,title=$data.title,npm=$data.npm,scripts=$data.scripts,meta=$data.meta,http=$data.http,equiv=$data.equiv,value=$data.value,link=$data.link,rel=$data.rel,type=$data.type,href=$data.href,script=$data.script,async=$data.async,src=$data.src,body=$data.body,div=$data.div,id=$data.id,h1=$data.h1,a=$data.a,p=$data.p,How=$data.How,handles=$data.handles,the=$data.the,quot=$data.quot,field=$data.field,h2=$data.h2,DESCRIPTION=$data.DESCRIPTION,supports=$data.supports,property=$data.property,of=$data.of,following=$data.following,ul=$data.ul,li=$data.li,prepublish=$data.prepublish,Run=$data.Run,BEFORE=$data.BEFORE,is=$data.is,published=$data.published,Also=$data.Also,run=$data.run,on=$data.on,local=$data.local,code=$data.code,install=$data.install,without=$data.without,any=$data.any,publish=$data.publish,postpublish=$data.postpublish,AFTER=$data.AFTER,preinstall=$data.preinstall,installed=$data.installed,postinstall=$data.postinstall,preuninstall=$data.preuninstall,uninstall=$data.uninstall,uninstalled=$data.uninstalled,postuninstall=$data.postuninstall,preupdate=$data.preupdate,updated=$data.updated,update=$data.update,command=$data.command,postupdate=$data.postupdate,pretest=$data.pretest,test=$data.test,posttest=$data.posttest,by=$data.by,prestop=$data.prestop,stop=$data.stop,poststop=$data.poststop,prestart=$data.prestart,start=$data.start,poststart=$data.poststart,prerestart=$data.prerestart,restart=$data.restart,postrestart=$data.postrestart,will=$data.will,and=$data.and,no=$data.no,provided=$data.provided,Additionally=$data.Additionally,arbitrary=$data.arbitrary,can=$data.can,be=$data.be,executed=$data.executed,running=$data.running,lt=$data.lt,pkg=$data.pkg,gt=$data.gt,stage=$data.stage,em=$data.em,Pre=$data.Pre,post=$data.post,commands=$data.commands,matching=$data.matching,names=$data.names,those=$data.those,as=$data.as,well=$data.well,e=$data.e,premyscript=$data.premyscript,myscript=$data.myscript,postmyscript=$data.postmyscript,NOTE=$data.NOTE,INSTALL=$data.INSTALL,SCRIPTS=$data.SCRIPTS,ARE=$data.ARE,AN=$data.AN,ANTIPATTERN=$data.ANTIPATTERN,strong=$data.strong,tl=$data.tl,dr=$data.dr,Don=$data.Don,t=$data.t,use=$data.use,file=$data.file,compilation=$data.compilation,anything=$data.anything,You=$data.You,should=$data.should,almost=$data.almost,never=$data.never,have=$data.have,to=$data.to,explicitly=$data.explicitly,set=$data.set,or=$data.or,you=$data.you,are=$data.are,doing=$data.doing,please=$data.please,consider=$data.consider,there=$data.there,another=$data.another,option=$data.option,The=$data.The,only=$data.only,valid=$data.valid,which=$data.which,must=$data.must,done=$data.done,target=$data.target,architecture=$data.architecture,early=$data.early,versions=$data.versions,node=$data.node,was=$data.was,often=$data.often,using=$data.using,waf=$data.waf,standalone=$data.standalone,Makefile=$data.Makefile,required=$data.required,that=$data.that,it=$data.it,This=$data.This,not=$data.not,portable=$data.portable,harder=$data.harder,properly=$data.properly,In=$data.In,current=$data.current,version=$data.version,standard=$data.standard,way=$data.way,extension=$data.extension,root=$data.root,your=$data.your,then=$data.then,appropriate=$data.appropriate,gyp=$data.gyp,automatically=$data.automatically,at=$data.at,time=$data.time,officially=$data.officially,supported=$data.supported,method=$data.method,compiling=$data.compiling,binary=$data.binary,addons=$data.addons,does=$data.does,require=$data.require,add=$data.add,If=$data.If,other=$data.other,things=$data.things,before=$data.before,used=$data.used,dependent=$data.dependent,operating=$data.operating,system=$data.system,instead=$data.instead,includes=$data.includes,tasks=$data.tasks,such=$data.such,Compile=$data.Compile,CoffeeScript=$data.CoffeeScript,source=$data.source,into=$data.into,JavaScript=$data.JavaScript,Create=$data.Create,minified=$data.minified,Fetching=$data.Fetching,remote=$data.remote,resources=$data.resources,advantage=$data.advantage,these=$data.these,they=$data.they,once=$data.once,single=$data.single,place=$data.place,thus=$data.thus,greatly=$data.greatly,reduce=$data.reduce,complexity=$data.complexity,variability=$data.variability,means=$data.means,depend=$data.depend,coffee=$data.coffee,devDependency=$data.devDependency,users=$data.users,don=$data.don,need=$data.need,include=function(filename,data){data=data||$data;var text=$utils.$include(filename,data,$filename);$out+=text;return $out;},minifiers=$data.minifiers,reducing=$data.reducing,size=$data.size,rely=$data.rely,having=$data.having,curl=$data.curl,wget=$data.wget,tools=$data.tools,machines=$data.machines,DEFAULT=$data.DEFAULT,VALUES=$data.VALUES,some=$data.some,values=$data.values,based=$data.based,contents=$data.contents,server=$data.server,clean=$data.clean,configure=$data.configure,build=$data.build,wscript=$data.wscript,compile=$data.compile,USER=$data.USER,invoked=$data.invoked,privileges=$data.privileges,change=$data.change,uid=$data.uid,user=$data.user,account=$data.account,specified=$data.specified,config=$data.config,defaults=$data.defaults,nobody=$data.nobody,unsafe=$data.unsafe,perm=$data.perm,flag=$data.flag,ENVIRONMENT=$data.ENVIRONMENT,Package=$data.Package,an=$data.an,environment=$data.environment,where=$data.where,many=$data.many,pieces=$data.pieces,information=$data.information,made=$data.made,available=$data.available,regarding=$data.regarding,setup=$data.setup,state=$data.state,process=$data.process,h3=$data.h3,path=$data.path,modules=$data.modules,define=$data.define,executable=$data.executable,like=$data.like,suites=$data.suites,executables=$data.executables,added=$data.added,PATH=$data.PATH,executing=$data.executing,has=$data.has,pre=$data.pre,name=$data.name,foo=$data.foo,dependencies=$data.dependencies,bar=$data.bar,could=$data.could,execute=$data.execute,exported=$data.exported,node_modules=$data.node_modules,directory=$data.directory,vars=$data.vars,fields=$data.fields,tacked=$data.tacked,onto=$data.onto,npm_package_=$data.npm_package_,prefix=$data.prefix,instance=$data.instance,had=$data.had,would=$data.would,npm_package_name=$data.npm_package_name,variable=$data.variable,npm_package_version=$data.npm_package_version,configuration=$data.configuration,Configuration=$data.Configuration,parameters=$data.parameters,put=$data.put,npm_config_=$data.npm_config_,view=$data.view,effective=$data.effective,checking=$data.checking,npm_config_root=$data.npm_config_root,Special=$data.Special,object=$data.object,keys=$data.keys,overwritten=$data.overwritten,param=$data.param,key=$data.key,example=$data.example,port=$data.port,behavior=$data.behavior,lifecycle=$data.lifecycle,event=$data.event,Lastly=$data.Lastly,npm_lifecycle_event=$data.npm_lifecycle_event,whichever=$data.whichever,cycle=$data.cycle,being=$data.being,different=$data.different,parts=$data.parts,switches=$data.switches,what=$data.what,s=$data.s,currently=$data.currently,happening=$data.happening,Objects=$data.Objects,flattened=$data.flattened,format=$data.format,so=$data.so,$out='';$out+=$escape(<!doctype html>
<html>
